# corotTruss 共转坐标桁架实验单元
## 命令
此命令用于构造共旋 truss 实验单元对象。共旋公式采用一组随单元旋转的共旋轴，从而考虑局部和全局参考系之间的精确几何变换。两个节点定义此单元。

**corotTruss（site）**
```tcl
expElement corotTruss eleTag iNode jNode -site siteTag -initStif Kij <-tangStif tangStifTag> <-iMod> <-noRayleigh> <-rho rho> <-cMass>
```

**corotTruss（server）**
```tcl
expElement corotTruss eleTag iNode jNode -server ipPort <ipAddr> <-ssl> <-udp> <-dataSize size> -initStif Kij <-tangStif tangStifTag> <-iMod> <-noRayleigh> <-rho rho> <-cMass>
```
...
节点 3 和 4 连接上述二维共旋 truss 实验单元。该单元使用本地实验站点定义。该单元的初始刚度为 [expr 3000.0*5.0/135.76] = 110.5。
$\mathbf { K } _ { i } = \left[ 110.5 \right]$

![corotTruss 实验单元](fig/corotTruss.jpg)

## 记录Recorder
在创建 ElementRecorder 对象时（参见 OpenSees 手册），对corotTruss实验单元的有效查询包括：
